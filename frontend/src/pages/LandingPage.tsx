...
import {
  ...
  CheckCircle, XCircle, ArrowRight, Menu, X, Star, AlertTriangle, ChevronDown,
} from 'lucide-react';
...
const FAQS = [
  {
    ...
    a: "When a call comes through to your TradeDesk number, our AI picks up in under 2 seconds. It introduces itself as your business, handles the conversation naturally, gives callers accurate quotes based on your pricing guide, and texts you a full summary.",
  },
  {
    q: 'What happens if I miss a call while on a job?',
    a: "TradeDesk answers it for you. The AI takes the caller's details, explains what they need, gives a rough quote, and tells them you'll follow up. You get an SMS summary with everything — name, number, what they need, and what was quoted. No lead lost.",
  },
  {
    q: 'Can I customise what the AI says?',
    a: "Yes. During setup you tell TradeDesk your business name, trade type, services, pricing guide, and working hours. The AI uses all of that to have natural, accurate conversations. You can update everything any time from your Settings page.",
  },
  {
  ...
  },
  {
    ...
    a: "The AI is trained to be honest when it doesn't have the answer. It will say something like \"I'd need to check that with Dave — can I get your number and he'll call you back?\" It never makes things up.",
  },
  {
    q: 'Can I cancel any time?',
    a: "Yes, absolutely. No lock-in contracts. Cancel from your account settings at any time and you won't be charged again. We also offer a 30-day money-back guarantee.",
  },
];
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="glass rounded-xl overflow-hidden border border-white/8 hover:border-white/15 transition-colors duration-200">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left"
      >
        <span className="font-medium text-white text-sm sm:text-base">{q}</span>
        <ChevronDown size={18} className={`text-gray-400 flex-shrink-0 transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${open ? 'rotate-180' : ''}`} />
      </button>
  ...
  return (
    <div className="relative mx-auto" style={{ width: 280 }}>
      ...
      <div className="relative bg-gray-900 rounded-[36px] p-2.5 border-2 border-white/15 shadow-2xl shadow-black/60">
        ...
        {/* Screen */}
        <div className="relative bg-[#0a0f1e] rounded-[28px] overflow-hidden" style={{ height: 520 }}>
          {/* Status bar */}
          <div className="flex items-center justify-between px-6 pt-10 pb-2 text-[10px] text-gray-400">
            <span>9:41</span>
            <span className="flex gap-1 items-center">
              <span className="w-3 h-1.5 border border-gray-400 rounded-sm"><span className="block bg-gray-400 h-full w-4/5" /></span>
            </span>
          </div>
          {/* Content */}
          <div className="px-3 pb-4 flex flex-col h-[calc(100%-48px)]">
            {children}
          </div>
        </div>
      </div>
...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans overflow-x-hidden">
      ...
      <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center transition-transform duration-200 hover:scale-110" style={{ boxShadow: '0 0 16px rgba(59,130,246,0.4)' }}>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link to="/signup" className="text-sm font-semibold bg-blue-500 hover:bg-blue-400 active:scale-[0.97] text-white px-4 py-2 rounded-lg transition-all duration-200" style={{ boxShadow: '0 0 16px rgba(59,130,246,0.35)' }}>Get Started</Link>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-black/95 backdrop-blur-md border-b border-white/8 px-5 py-4 space-y-4 animate-fade-in">
            ...
            <div className="flex gap-3 pt-2">
              <Link to="/login" className="flex-1 text-center text-sm glass rounded-lg py-2.5 text-gray-300" onClick={() => setMenuOpen(false)}>Log in</Link>
              <Link to="/signup" className="flex-1 text-center text-sm bg-blue-500 hover:bg-blue-400 rounded-lg py-2.5 font-semibold transition-colors" onClick={() => setMenuOpen(false)}>Get Started</Link>
            </div>
          </div>
        )}
      </nav>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center lg:items-start">
                <Link to="/signup"
                  className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-400 active:scale-[0.97] text-white font-semibold px-8 py-4 rounded-xl text-base transition-all duration-200 w-full sm:w-auto justify-center"
              ...
              <PhoneFrame>
                <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-white/8 flex-shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-green-400">● Online · answering now</p>
                  </div>
                </div>
                <div className="space-y-2 flex-1 overflow-hidden">
                  <ChatBubble role="ai" text="G'day! You've reached Smith's Plumbing. Dave's on a job — I'm his AI receptionist. How can I help?" />
                  <ChatBubble role="caller" text="I've got a burst pipe in the kitchen. It's pretty urgent." />
                  {showTyping ? (
                    <ChatBubble role="ai" text="No worries! A burst pipe repair usually runs $180–$320. I'll mark this urgent and Dave will call you back within 30 minutes. Can I grab your name and number?" />
                  ) : (
                  ...
                  {showTyping && (
                    <div className="text-center py-2 animate-fade-in">
                      <span className="text-[10px] text-gray-600">Dave gets an SMS summary instantly</span>
                    </div>
                  )}
                </div>
              </PhoneFrame>
      ...
      {/* TRUSTED BY TRADIES */}
      <section className="py-12 px-5 border-y border-white/6">
        <div className="max-w-5xl mx-auto">
          <p className="text-center text-xs text-gray-600 font-semibold uppercase tracking-widest mb-8">Trusted by tradies across Australia</p>
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-12 gap-3">
            {TRADE_ICONS.map(({ label, emoji }) => (
              <div key={label} className="flex flex-col items-center gap-1.5 group cursor-default">
                <div className="w-10 h-10 glass rounded-xl flex items-center justify-center text-xl group-hover:border-white/20 group-hover:bg-white/5 transition-all duration-200">
                  {emoji}
                </div>
                <span className="text-[9px] text-gray-600 group-hover:text-gray-400 transition-colors whitespace-nowrap">{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {[
              ...
              { n: '3', title: 'You get a text', desc: "After each call you get an SMS with the caller's name, number, what they needed, and what was quoted." },
            ].map(step => (
              <div key={step.n} className="text-center">
                <div className="w-12 h-12 bg-blue-500/15 border border-blue-500/30 rounded-2xl flex items-center justify-center mx-auto mb-5">
          ...
          <div className="mt-10 flex items-start gap-4 glass border border-red-500/20 rounded-2xl p-5 max-w-2xl mx-auto">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">If a caller says it's urgent, the AI flags it and texts you an urgent alert so no emergency ever slips through.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-5 relative">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {[
              { name: 'Marcus Webb', trade: 'Plumber', location: 'Brisbane, QLD', quote: "I was losing 3–4 jobs a week because I couldn't answer calls on the job. TradeDesk fixed that overnight. Made back the subscription cost in the first week.", stars: 5 },
              { name: 'Tanya Kowalski', trade: 'Electrician', location: 'Sydney, NSW', quote: "Set it up in about 8 minutes. Now I check my phone at smoko and I've already got a list of leads with their details and what they need. It's unreal.", stars: 5 },
              { name: 'Brett Sullivan', trade: 'Builder', location: 'Melbourne, VIC', quote: "I was sceptical about AI but this actually sounds professional. My clients have no idea they're not talking to a real receptionist. Highly recommend.", stars: 5 },
      ...
      <section id="features" className="py-24 px-5 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { icon: Phone, title: 'AI answers calls 24/7', desc: 'Never miss a lead. Your AI picks up in under 2 seconds, any time of day or night.', color: 'text-blue-400', bg: 'bg-blue-500/15' },
              { icon: MessageSquare, title: 'Instant SMS summaries', desc: 'After every call: caller number, what they needed, quotes given, what was agreed.', color: 'text-green-400', bg: 'bg-green-500/15' },
              { icon: Mail, title: 'Email auto-reply', desc: 'Connect Gmail and the AI replies to enquiry emails automatically.', color: 'text-purple-400', bg: 'bg-purple-500/15' },
              { icon: Clock, title: 'Missed call text-back', desc: 'If a call goes unanswered, the caller gets a text within 60 seconds.', color: 'text-yellow-400', bg: 'bg-yellow-500/15' },
              { icon: FileText, title: 'Full call transcripts', desc: 'Every conversation is recorded and searchable in your dashboard.', color: 'text-pink-400', bg: 'bg-pink-500/15' },
              { icon: Zap, title: 'Live in 10 minutes', desc: 'No hardware, no technical setup. Enter your details and forward your calls.', color: 'text-orange-400', bg: 'bg-orange-500/15' },
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="glass rounded-2xl overflow-hidden border border-white/10">
            <div className="grid grid-cols-3 text-sm font-semibold border-b border-white/10">
              ...
              <div className="py-4 px-4 text-gray-500 text-center">Human Receptionist</div>
            </div>

            {[
              { label: 'Monthly cost', td: '$199/mo', hr: '$4,000–$6,000/mo', tdCheck: true, hrCheck: false },
              { label: 'Availability', td: '24/7 including weekends', hr: 'Business hours only', tdCheck: true, hrCheck: false },
              { label: 'Answer time', td: 'Under 2 seconds', hr: 'Varies — often on hold', tdCheck: true, hrCheck: false },
              { label: 'Sick days', td: 'Zero — ever', hr: '10 days/year average', tdCheck: true, hrCheck: false },
              { label: 'Accurate quotes', td: 'Always consistent', hr: 'Depends on training', tdCheck: true, hrCheck: false },
              { label: 'SMS summary', td: 'After every call', hr: 'Not typically', tdCheck: true, hrCheck: false },
              { label: 'Setup time', td: '10 minutes', hr: 'Weeks of hiring & training', tdCheck: true, hrCheck: false },
              { label: 'Scales with volume', td: 'Unlimited calls', hr: 'One call at a time', tdCheck: true, hrCheck: false },
            ].map(({ label, td, hr, tdCheck, hrCheck }, i) => (
              <div key={label} className={`grid grid-cols-3 text-sm border-b border-white/5 last:border-0 ${i % 2 !== 0 ? 'bg-white/[0.015]' : ''}`}>
                <div className="py-3.5 px-4 text-gray-400 pl-6">{label}</div>
                <div className="py-3.5 px-4 bg-blue-500/5 border-x border-blue-500/10 flex items-center justify-center gap-1.5 text-white font-medium">
                  {tdCheck && <CheckCircle size={14} className="text-green-400 flex-shrink-0" />}
                  <span className="text-xs sm:text-sm">{td}</span>
                </div>
                <div className="py-3.5 px-4 flex items-center justify-center gap-1.5 text-gray-500">
                  {!hrCheck && <XCircle size={14} className="text-red-400/70 flex-shrink-0" />}
          ...
          <div className="text-center mt-8">
            <Link to="/signup"
              className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-400 active:scale-[0.97] text-white font-semibold px-7 py-3.5 rounded-xl transition-all duration-200"
      ...
      <section className="py-10 overflow-hidden border-y border-white/6">
        <div className="flex whitespace-nowrap gap-12" style={{ animation: 'marquee 25s linear infinite' }}>
          {[...Array(4)].map((_, i) =>
            ['Plumber', 'Electrician', 'Builder', 'Carpenter', 'Painter', 'Roofer', 'Landscaper', 'Tiler', 'Locksmith', 'HVAC tech', 'Concreter', 'Pest Control'].map(trade => (
          ...
          <div className="max-w-md mx-auto">
            <div className="relative">
              ...
              <div className="relative glass rounded-2xl p-8">
                ...
                <Link to="/signup"
                  className="flex items-center justify-center gap-2 w-full bg-blue-500 hover:bg-blue-400 active:scale-[0.98] text-white font-semibold py-3.5 rounded-xl transition-all duration-200"
                  style={{ boxShadow: '0 0 20px rgba(59,130,246,0.3)' }}>
            ...
            <div className="mt-6 flex items-center justify-center gap-3 glass rounded-xl px-5 py-4 border border-green-500/20">
              ...
              <p className="text-sm text-gray-400">
                ...
                If TradeDesk doesn't pay for itself, we'll refund you in full.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 px-5 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/signup"
              className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-400 active:scale-[0.98] text-white font-semibold px-8 py-4 rounded-xl text-base transition-all duration-200"
      ...
      <footer className="border-t border-white/8 py-10 px-5">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            <Link to="/login" className="text-sm text-gray-600 hover:text-white transition-colors">Log in</Link>
            <Link to="/signup" className="text-sm bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 px-3 py-1.5 rounded-lg transition-all duration-200">Get Started</Link>
          </div>
        </div>
      </footer>
      ...
      <div className={`fixed bottom-0 inset-x-0 z-40 md:hidden transition-all duration-300 ${showScrollCTA ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
        <div className="bg-black/90 backdrop-blur-md border-t border-white/10 px-4 py-3 flex items-center gap-3">
          ...
          <Link to="/signup"
            className="flex-shrink-0 bg-blue-500 hover:bg-blue-400 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-all duration-200 active:scale-[0.97]">
            Get started
          </Link>
        </div>
      </div>
