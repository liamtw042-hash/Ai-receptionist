...
// (OpenAI). So it's deliberately locked down: short inputs, short outputs, and a
// per-IP daily cap so a bot can't run up an unbounded bill.
...
router.post('/widget', async (req: Request, res: Response) => {
  ...
  // Sanitise history to the exact shape getAIResponse expects, bounded in length.
